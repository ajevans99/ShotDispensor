"""
This file is from https://github.com/davidjamesmoss/python-magstripe.git
and was modified to support Michigan State University ID cards
"""
...
class MagStripe():
    ...
    def parseMSU(self, trackstr):
        if not trackstr:
            raise MagStripeError('Blank msu track data')
        trackdata = trackstr[1:len(trackstr)-1]  # remove start/end sentinel

        try:
            _, name, pid = trackdata.split('^')
            pid = pid[7:16]
        except:
            raise MagStripeError('Fail to parse msu track')

        try:
            lastname, firstname = name.split('/')
        except:
            raise MagStripeError('Fail to parse first and last name from MSU ID')
    ...
    def parse(self, input):
        ...
        if self.msuIDPrefix in track1:
            print("MSUID!")
            return self.parseMSU(track1)
